#!/usr/bin/env python3
"""
Upload Infinite HD chunks to R2 bucket. Resume-capable.

Progress: tail /tmp/infinite-hd-upload.log
Status:   python3 scripts/upload-infinite-hd.py --status
"""
...
MANIFEST = os.path.join(os.path.dirname(__file__), "..", "src", "Data", "Infinite HD.dsk.json")
BUILD_DIR = os.path.join(os.path.dirname(__file__), "..", "Images", "build")
R2_BUCKET = "sandmill-mac-disk"
LOG_FILE = "/tmp/infinite-hd-upload.log"
DONE_FILE = "/tmp/infinite-hd-upload-done.txt"
...
def status():
    with open(MANIFEST) as f:
        d = json.load(f)
    chunks = list(set(c for c in d["chunks"] if c))
    done = load_done()
        ...
        with open(DONE_FILE, "a") as done_f:
            for i, chunk in enumerate(remaining):
                ...
                if not os.path.exists(chunk_file):
                    log.write(f"MISSING [{i+1}/{len(remaining)}] {chunk}.chunk\n")
                    log.flush()
                    continue
